...
for dir in Processed_htm_audios-*; do
    ...
    for file in "$dir"/*.htm; do
        ...
        cp "$file" "$TMP_BIN/$categoria/$new_filename"
    done
done

echo "Subiendo archivos al servidor remoto demo:~/bin/..."
# Subir toda la estructura de /tmp/bin/* al servidor destino usando scp -r
scp -r "$TMP_BIN"/* "$REMOTE_SERVER:~/bin/"

# Limpiar el directorio temporal local
echo "Limpiando directorio temporal..."
rm -rf "$TMP_BIN"

echo "¡Proceso de subida finalizado con éxito!"
