...
if [ -z "$PORTABLE_MODE" ]; then
    ...
    PIPER_EXE="piper"
    PIPER_MODEL_DIR="$PORTABLE_ROOT/portable-bin-PATH/share/piper"
fi
...
if true; then
    ...
    cat /tmp/osd_text.tmp | "$PIPER_EXE" --model "$PIPER_MODEL_DIR/$VERSION" --output_file "$TMP_WAV" >/dev/null 2>&1
