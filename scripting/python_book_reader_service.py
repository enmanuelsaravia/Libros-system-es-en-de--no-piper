...
# --- CONFIGURACIÓN ---
PATTERN = re.compile(
    r"^bash\s+(?:~/|/home/user/)monoliths-llm/vociferate-pdf\.sh\s+((?:~/|/home/user/)Libros/.+\.pdf)$"
)
    ...
    try:
        ...
        if text != last_text:
            ...
            if match:
                ...
                # Encontrar directorio del script y del bundle portátil
                script_dir = Path(__file__).resolve().parent
                portable_root = script_dir.parent

                # Resolver ruta absoluta del PDF en el directorio portable
                if pdf_path.startswith("~/Libros/"):
                    real_pdf_path = portable_root / pdf_path[len("~/Libros/"):]
                elif pdf_path.startswith(f"{PROJECT_ROOT}/"):
                    real_pdf_path = portable_root / pdf_path[len(f"{PROJECT_ROOT}/"):]
                else:
                    real_pdf_path = Path(os.path.expanduser(pdf_path))

                # Lanzar nuevo proceso usando el script del bundle portable
                cmd = f"bash {script_dir}/vociferate-pdf.sh {real_pdf_path}"
